...
- **transition-patterns** — Nine proven CSS transitions (modal, dropdown, panel, page, etc.) remapped to frilo tokens.
- **ui-design-principles** — The general grammar underneath the token system: grids, type scales, contrast, shadows, component anatomy, imagery. Frilo tokens win on concrete values; this covers what they leave undefined and doubles as the audit layer.
- **ai-interface-patterns** — Design patterns for AI-powered features: chat vs copilot vs agent, generative UI, streaming, citations, confidence, permissions, disclosure. Use for any feature where a model generates, decides, or acts.
- **micro-interactions** — The half-second after a click: easing curves, spring tuning, gesture abandonment, focus shapes, optimistic rollback, layout-shift prevention. More specific than ui-design-principles for component-level motion.
- **ai-ux-rules** — 72 numbered, checkable rules distilled from ai-interface-patterns and micro-interactions, each with a threshold or concrete behavior. Build spec and review protocol in one; feed rule numbers straight to a coding agent.
- **untitled-ui** — Bridge to the Untitled UI icon and component library. Icons always come from `@untitledui/icons`, never a one-off SVG; components are scaffolding only, restyled onto frilo tokens before they ship.
- **dialkit** — Bridge to DialKit, a live-tuning panel for motion, spacing, and color values. Dev-only; whatever gets tuned still gets written back into TOKENS.json as the source of truth.
- **muload** — Bridge to muload's AI-shaped loading and streaming patterns (token streams, tool calls, agent runs). For the waits ordinary spinners and skeletons don't cover.
- **guideframe** — Bridge to GuideFrame, a browser grid/ruler/inspector overlay for verifying a built screen against its actual grid math. Dev-only audit tool.
...
- **Jakubantalik/transitions.dev** — ships its own skill now (`reveal`/`review`/`apply`/`refine`/`polish` over 27+ transitions). The local `transition-patterns` skill is just the remap step onto frilo tokens that runs after it.
- **index-how/vocabulary** (index.how) — a 188-term design vocabulary across typography, color, motion, IA, and more. Most sections stay locked until their fall 2026 launch; installed now so it updates in place as they unlock.
